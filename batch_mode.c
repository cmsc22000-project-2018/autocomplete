...
//	char** children = trie_approx(new_dict->dict, s, 2, n); 
	...
	log_debug("get_n_children_in_dict: about to call dict_suggestions");
...
//TODO:	assert ((trie_contains(d->dict, s) == 2) || (trie_contains(d->dict, s) == 0));
	log_debug("num_children_in_dict: about to call trie_completions");
	int c = trie_completions(d->dict, s);
//	printf("num_ch_d: about to assert d!=NULL\n");
//	assert(d != NULL);
//	printf("num_ch_d: successful assetion that d!=NULL\n");
//	int c = strlen(s);
	log_trace("num_chren_in_dict:finished trie_completions, value= %d", c); 
	return c;
}
...
void print_children(int b, int n, char* s, dict_t* dict)
{
	...
	log_debug("print_children: about to call get_n_ch_d and num_ch_d\n");
    char** children = get_n_children_in_dict(str, dict,n);
    int num_children = num_children_in_dict(str, dict);
	log_debug("print_children: about to call prefix_new, word is %s\n", s);
    prefix = prefix_new(s, children, num_children);
	log_debug("print_children: finished calling prefix_new\n");
	...
	log_debug("print_children: about to call prefix_free before exiting\n");
    prefix_free(prefix);
...
//					dictionary = calloc(sizeof(args[i+1]) + 1, sizeof(char));
	...
	log_debug("batch_mode_builtin: about to call new_dictionary_from_file\n");
