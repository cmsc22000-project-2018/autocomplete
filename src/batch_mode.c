...
#define DEFAULTDICT "./src/case_dict.txt"
...
void print_children(int b, int n, char* s, char* dict_file)
{
    ...
    char** children = get_children_in_dict(str, dict_file);
...
int batch_mode_builtin(char **args)
{
	...
	char* dictionary = DEFAULTDICT; //Once we can, should be initialized to the redis dictionary
	FILE* prefixFile;
	int fileSet = 0;
